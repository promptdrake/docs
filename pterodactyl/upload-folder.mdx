...
<Warning>
  - Do not upload while server status is Installing
  - Do not upload while server status is Suspended
  - Use a stable connection
  - Avoid uploading large files at once; never upload node\_modules to the panel
</Warning>